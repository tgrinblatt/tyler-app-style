...
- **Never hardcode hex strings in views** — always use `NothingTokens.Color.*`
...
- **Never use `.background(.regularMaterial)` or `.background(.ultraThinMaterial)`** on content areas — materials are for chrome only
...
- **Never use `.font(.body)` or `.font(.headline)`** — always use `NothingTokens.Font.*`
- **Never use lowercase for labels/section headers** — always ALL-CAPS with 0.88 tracking via `.nothingLabel()`
...
- **Never use arbitrary spacing values** — always use `NothingTokens.Spacing.*` (multiples of 8)
